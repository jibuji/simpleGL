...
public class GLES20Canvas implements GLCanvas {
    ...
    private static final String TINT_UNIFORM = "uTint";
    private static final String TINT_TEXTURE_FRAGMENT_SHADER = ""
            + "precision mediump float;\n"
            + "varying vec2 vTextureCoord;\n"
            + "uniform vec4 " + TINT_UNIFORM + ";\n"
            + "uniform float " + ALPHA_UNIFORM + ";\n"
            + "uniform sampler2D " + TEXTURE_SAMPLER_UNIFORM + ";\n"
            + "void main() {\n"
            + "  vec4 color = texture2D(" + TEXTURE_SAMPLER_UNIFORM + ", vTextureCoord);\n"
            + "  color.rgb = " + TINT_UNIFORM + ".rgb * color.a;\n"
            + "  color *= " + ALPHA_UNIFORM + ";\n"
            + "  gl_FragColor = color;\n"
            + "}\n";

    ShaderParameter[] mTintTextureParameters = {
            new AttributeShaderParameter(POSITION_ATTRIBUTE), // INDEX_POSITION
            new UniformShaderParameter(MATRIX_UNIFORM), // INDEX_MATRIX
            new UniformShaderParameter(TEXTURE_MATRIX_UNIFORM), // INDEX_TEXTURE_MATRIX
            new UniformShaderParameter(TEXTURE_SAMPLER_UNIFORM), // INDEX_TEXTURE_SAMPLER
            new UniformShaderParameter(ALPHA_UNIFORM), // INDEX_ALPHA
            new UniformShaderParameter(TINT_UNIFORM), // INDEX_TINT
    };
    ...
    private static final int INDEX_TINT = 5;
    // Handle indices -- mesh
    ...
    private int mTintedTextureProgram;
    ...
    public GLES20Canvas() {
        ...
        int tintedTextureFragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER,
                TINT_TEXTURE_FRAGMENT_SHADER);
        ...
        mTintedTextureProgram = assembleProgram(textureVertexShader,
                tintedTextureFragmentShader, mTintTextureParameters);
    ...
    private float[] getPureColor(int color) {
    ...
    @Override
    public void drawTintedTexture(BasicTexture texture, int tColor, int x, int y, int width, int height) {
        if (width <= 0 || height <= 0) {
            return;
        }
        copyTextureCoordinates(texture, mTempSourceRect);
        RectF target = mTempTargetRect;
        target.set(x, y, x + width, y + height);
        convertCoordinate(mTempSourceRect, target, texture);
        setTextureMatrix(mTempSourceRect);

        prepareTexture(texture, mTintedTextureProgram, mTintTextureParameters);
        ShaderParameter[] params = mTintTextureParameters;
        float[] colorArray = getColor(tColor);
        GLES20.glUniform4fv(params[INDEX_TINT].handle, 1, colorArray, 0);

        setPosition(params, OFFSET_FILL_RECT);
        GLES20.glUniformMatrix4fv(params[INDEX_TEXTURE_MATRIX].handle, 1, false, mTempTextureMatrix, 0);
