...
public class GLView {
    ...
    private static boolean dispatchTouchEvent(MotionEvent event,
    ...
    private static void cancelView(GLView target, MotionEvent event) {
        int x = (int) event.getX();
        int y = (int) event.getY();
        cancelView(target, event, x, y);
    }

    private static void cancelView(GLView target, MotionEvent event, int x, int y) {
        MotionEvent cancel = MotionEvent.obtain(event);
        cancel.setAction(MotionEvent.ACTION_CANCEL);
        dispatchTouchEvent(cancel, x, y, target, false);
        cancel.recycle();
    }
    ...
    public void attachToRoot(GLRootView root) {
    ...
    //TODO
    final boolean dispatchTouchEvent(MotionEvent event) {
        if (onInterceptTouchEvent(event)) {
            if (mMotionTarget != null) {
                cancelView(mMotionTarget, event);
                mMotionTarget = null;
            }
            return true;
        }
        ...
        if (mMotionTarget != null) {
            if (action == MotionEvent.ACTION_DOWN) {
                cancelView(mMotionTarget, event, x, y);
                mMotionTarget = null;
            } else {
